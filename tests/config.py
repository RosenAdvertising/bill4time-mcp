...
from bill4time_mcp.server import mcp
...
TOOLKIT = ToolkitConfig(
    ...
    source_path=_TESTS_DIR.parent / "bill4time_mcp",
    module_path="bill4time_mcp",
    server_path=_TESTS_DIR.parent / "bill4time_mcp" / "server.py",
    resilience=ResilienceConfig(tools_to_timeout_test=["list_clients"]),
    skip_tiers={
        "contract": "no published OpenAPI spec for Bill4Time API (HTML docs only)",
