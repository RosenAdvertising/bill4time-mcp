...
Storage order (read):  OS keyring  ->  process env  ->  ~/.bill4time-mcp/.env file
...
user opts out of keyring with ``BILL4TIME_MCP_USE_KEYRING=0``.
...
_USE_KEYRING_FLAG = "BILL4TIME_MCP_USE_KEYRING"
# ---------------------------------------------------------------------------
...
def _keyring_enabled() -> bool:
    ...
    Opt out with ``BILL4TIME_MCP_USE_KEYRING=0`` (or false/no/off). A failing or
